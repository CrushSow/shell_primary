...
> sed常见的语法格式有两种，一种叫==命令行==模式，另外一种叫==脚本==模式
...
sed -i "/HOSTNAME/cHOSTNAME=server$iptail.itcast.com"
/etc/sysconfig/network
echo "$ipaddr server$iptail.itcast.com"
...
sed -ir '/^# | ^$/d' $conf
sed -i '/local_enable/c\local_enabled=NO' $conf
...
# 启动服务
service vsftpd restart &>/dev/null && echo "vsftpd 服务启动成功"
